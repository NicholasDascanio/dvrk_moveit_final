...
#with this flag i'm sure that the trajectory is feasibile and that the retiming can be done, otherwise i compute once again the traj. It's a check 
#on the feasibility of the trajectory. If a single point of the cartesian trajectory collides with an obstacle present on the scene, that means that
#it cannot be retimed and the trajectory should be computed once again
    while(flag==0):
...
# if i succeded in converting all the points to the joint space, that means that the trajectory is feasibile, so every point doesn't collide with obstacles
#then i can do retiming by setting the ratio of maximum velocity and acceleration
    if flag==1:
  ...
  def from_smooth_to_joint(self, traj_smooth=[], waypoints=[]): #function which tranforms the smooth trajectory from cartesian to joint
    ...
    for i in range(0,len(traj_smooth)):
      ...
      print(i)
      flag=1
      #when the trajectory is not feasible, the ik conversion gets slower, and it ends up with a segmentation fault error. In order to avoid that
      #i assume that if the conversion takes longer than 30 seconds, the trajectory is not feasible and should be recomputed      
